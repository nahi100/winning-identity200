...
// ════════════════════════════════════════════════════════════════════
//  STANDALONE MODE — saves to browser localStorage (per device).
//  No backend needed. Each browser/device gets its own data.
// ════════════════════════════════════════════════════════════════════
const STORAGE_KEY = "wi_user_data_v1";
...
const SEED_IDENTITIES = [];  // Empty — users define their own via the onboarding screen

const SEED_TASKS = [];  // Empty — users add their own tasks via the morning flow
...
export default function App() {
  ...
  const [loading, setLoading] = useState(true);        // initial auth/load
  const [onboarding, setOnboarding] = useState(false); // first-time identity setup
  ...
  useEffect(() => {
    ...
    if (!saved) {
      // First time on this browser — show onboarding
    ...
    if (saved.identities) setIdentities(saved.identities);
    if (saved.tasks) setTasks(saved.tasks);
    if (saved.todayEnergy) setTodayEnergy(saved.todayEnergy);
    if (saved.watchOut) setWatchOut(saved.watchOut);
    if (saved.energyLog) setEnergyLog(saved.energyLog);
    if (saved.readArticles) setReadArticles(new Set(saved.readArticles));
    // Check if today's morning is done — if yes, don't open the morning flow
    const today = new Date().toDateString();
    if (saved.lastMorningDate === today) setMorningOpen(false);
    setLoading(false);
  }, []);
  ...
  useEffect(() => {
    ...
    saveToStorage({
      identities, tasks, todayEnergy, watchOut, energyLog,
      readArticles: Array.from(readArticles),
      lastMorningDate: morningOpen ? null : new Date().toDateString(),
    });
  }, [identities, tasks, todayEnergy, watchOut, energyLog, readArticles, morningOpen, loading, onboarding]);

  function finishOnboarding(names) {
    const palette = [T.gold, T.green, T.blue, T.violet, T.red];
    const icons = ['🎯', '💪', '🏡', '📚', '🚀'];
    const newIds = names.filter((n) => n && n.trim()).map((n, i) => ({
      id: `id_${Date.now()}_${i}`, name: n.trim(), icon: icons[i] || '🎯',
      color: palette[i] || T.gold, level: 1, pts: 0, max: 300,
    }));
    setIdentities(newIds);
    setOnboarding(false);
    setMorningOpen(true);
    showToast(`נוצרו ${newIds.length} זהויות · קדימה לבוקר`);
  }
  ...
  // ── Early returns for loading / error / onboarding states ──
  if (loading) {
    return (
      <div dir="rtl" style={{ ...SS.root, alignItems: "center", justifyContent: "center" }}>
        <style>{CSS}</style><FontLink />
        <div style={{ textAlign: "center", padding: 40 }}>
          <div style={{ fontSize: 44, marginBottom: 16 }}>⚡</div>
          <div style={{ fontSize: 18, fontWeight: 700, color: T.gold, marginBottom: 8 }}>טוען...</div>
          <div style={{ fontSize: 13, color: T.soft }}>השרת מתעורר — עד דקה בפעם הראשונה</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div dir="rtl" style={{ ...SS.root, alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ textAlign: "center", padding: 40, maxWidth: 360 }}>
          ...
          <div style={{ fontSize: 17, fontWeight: 700, marginBottom: 10 }}>בעיית חיבור לשרת</div>
          <div style={{ fontSize: 13, color: T.soft, marginBottom: 20, lineHeight: 1.6 }}>{serverError}</div>
          <button onClick={() => window.location.reload()} style={{ ...SS.primaryBtn, width: "auto", padding: "12px 24px" }}>נסה שוב</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div dir="rtl" style={SS.root}>
      ...
      {morningOpen && <MorningFlow mode={morningOpen} identities={identities} cfg={cfg} initialEnergy={todayEnergy}
        ...
        onComplete={({ energy, picks, watch }) => { setTodayEnergy(energy); setEnergyLog((p) => [...p.slice(-5), energy]); picks.forEach((p) => addTask(p)); setWatchOut(watch); setMorningOpen(false); showToast("הבוקר שלך מוכן · קדימה לנצח"); }} />}
...
function Onboarding({ onComplete }) {
  ...
  const placeholders = [
    "למשל: איש משפחה טוב יותר",
    "למשל: סוחר מנצח / יזם",
    "למשל: ספורטאי בריא וחזק",
  ];
  function update(i, v) { setNames((p) => p.map((x, idx) => (idx === i ? v : x))); }
  const filled = names.filter((n) => n.trim()).length;

  return (
    <div dir="rtl" style={SS.root}>
      <style>{CSS}</style><FontLink />
      <div style={{ ...SS.frame, padding: "40px 24px" }}>
        <div className="wi-fade" style={{ marginTop: 30 }}>
          <div style={{ textAlign: "center", marginBottom: 30 }}>
            <div style={{ fontSize: 50, marginBottom: 14 }}>✦</div>
            <h1 style={{ fontSize: 26, fontWeight: 800, marginBottom: 10, color: T.text }}>
              ברוך הבא, אלוף
            </h1>
            <p style={{ fontSize: 14, color: T.soft, lineHeight: 1.6, maxWidth: 360, margin: "0 auto" }}>
              לפני שמתחילים — <b style={{ color: T.gold }}>מי האדם שאתה רוצה להפוך להיות?</b><br />
              בחר 3 זהויות שיגדירו את המסע שלך. כל פעולה שתעשה תבנה אותן.
            </p>
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 20 }}>
            {names.map((name, i) => (
              <div key={i} className="wi-rise" style={{ animationDelay: `${i * 0.08}s` }}>
                <div style={{ fontSize: 12, color: T.gold, fontWeight: 700, marginBottom: 6, paddingRight: 4 }}>
                  זהות {i + 1}
                </div>
                <input
                  type="text"
                  ...
                  style={{
                    ...
                    border: `1.5px solid ${name ? T.gold : T.line2}`, borderRadius: 12,
                    color: T.text, fontFamily: FONT, fontSize: 15, outline: "none", textAlign: "right",
          ...
          <div style={{ marginTop: 30 }}>
            <button
              ...
              className="wi-press"
              style={{ ...SS.primaryBtn, ...(filled === 0 ? { opacity: 0.4 } : {}) }}
            >
              {filled === 0 ? "מלא לפחות זהות אחת" : `התחל את המסע (${filled})`}
            </button>
            <p style={{ fontSize: 11, color: T.mute, textAlign: "center", marginTop: 14, lineHeight: 1.5 }}>
              אפשר לערוך, להוסיף ולמחוק זהויות בכל זמן
            </p>
          </div>
        </div>
      </div>
...
function Home({ cfg, tasks, identities, doneCount, totalPts, dayScore, streak, watchOut, wisdom, idMap, onToggle, onAdd, onFocus, onPanic, onMorning, onSummary, onEditIdent, onNewIdent, onTestNotif }) {
  ...
  return (
    <div className="wi-fade">
      <div style={SS.head}>
        <div><div style={SS.greetSmall}>בוקר טוב,</div><div style={SS.greetName}>אלוף <span style={{ color: T.gold }}>◆</span></div></div>
...
function Coach({ cfg, identities, doneCount, total, streak, energy }) {
  ...
  async function send(text) {
    ...
    try {
      // Direct call to Claude (works when running in preview env or with key in domain).
